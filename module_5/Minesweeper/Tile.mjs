...
export class TTile extends libSprite.TSpriteButton {
  ...
  onMouseUp(aEvent) {
    ...
    this.disable = true;
    // TODO: Sjekk om spillet er vunnet! (Hint: Bruk forEachTile)
    gameProps.openTiles = 0; //Vi må resette denne, later som om vi ikke har noen åpne enda
    forEachTile(this.#countOpenTiles);
    //Her er openTiles oppdatert med antall åpne miner.
    //Lag en if-test sjekk om flere tiles å åpne. Hvis ikke så er spillet slutt.
   
    const totalTiles = gameLevel.Tiles.Row * gameLevel.Tiles.Col;
    const tilesLeft = totalTiles - gameProps.openTiles - gameLevels.Mines;

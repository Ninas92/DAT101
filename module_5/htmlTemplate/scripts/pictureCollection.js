...
class TPictureCarousel extends TBootstrapComponent {
  ...
  render() {
    ...
    const btnSlideTypes = this.shadowRoot.querySelectorAll("input[name='btn-slide-type']");
    for(let i = 0; i < btnSlideTypes.length; i++) {
        const btn = btnSlideTypes[i];
        btn.addEventListener("change", this.#onSlideTypeChange.to(i));
    }
  }
}
